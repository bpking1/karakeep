...
- 本文词汇弹卡使用可用高度，统计/搜索/状态和标签筛选移入 FlatList 的可滚动头部；两排横向筛选禁止纵向收缩，中间列表分配剩余高度，批量操作保留在底部。选择按 termKey 保存、切标签保留勾选等语义不变，其它词卡尺寸不变。
- 本次新增缓存重复开篇、页面焦点恢复、设置就绪/禁用分支和筛选布局结构的隔离回归测试，合计 63 项通过；移动端 TypeScript、改动范围 Oxlint/Oxfmt 与 diff 检查通过。不以 React/静态测试替代 Android WebView 与小屏键盘布局验收。未运行端到端、原生 APK 编译或真实后端/AI。
